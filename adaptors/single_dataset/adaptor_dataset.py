...
class AdaptorDataset(Dataset):
    def __init__(self, csv_file, embeddings_file, target_column='Label', task_type='regression', transform=None,
        ...
        """
        # Load the CSV file
        if data not in ['all', 'summer', 'winter']:
        ...
        if data == 'summer':
            self.targets = self.targets[1705:]
            self.data_df = self.data_df[1705:]
            self.embeddings = self.embeddings[1705:]
        elif data == 'winter':
            self.targets = self.targets[:1705]
            self.data_df = self.data_df[:1705]
            self.embeddings = self.embeddings[:1705]
    ...
    def __getitem__(self, idx):
        ...
        file_name = self.data_df.iloc[idx]['Date']
        label = self.data_df.iloc[idx]['Label']
        sample = {'embedding': embedding, 'target': target_tensor, 'file_name': file_name, 'label': label}

        if self.task_type == 'classification':
            assert target_tensor.item() == int(self.folder_drug_dict[file_name]['label'])
