...
def get_valid_palette(num_colors, max_attempts=50):
    ...
    raise ValueError(
        "Could not generate a valid palette without light colors."
    )  # Fail-safe
...
def get_phenotypic_colors(similarity_matrix, num_clusters):
    ...
    np.fill_diagonal(
        similarity_matrix, 1
    )  # Set diagonal to 1 for perfect self-similarity
    ...
    def get_shaded_colors(
        base_color, num_shades, min_lightness=0.25, max_lightness=0.45
    ):
    ...
    class_colors = {}
    cluster_counts = {
        i: 0 for i in range(1, num_clusters + 1)
    }  # Track how many classes per cluster
    for i, cluster_id in enumerate(clusters):
        num_in_cluster = sum(
            clusters == cluster_id
        )  # Count classes in the same cluster
    ...
    to_print_dict = {
        ...
        5: "20240805 h2o2 5",
        6: "20240806 mitoq 6",
        7: "20240807 resveratrol 7",
        8: "20240808 lonidmaine 8",
        9: "20240809 oligomycin 9",
        10: "20240813 dnp 10",
        11: "20240814 valinomycin 11",
        12: "20240815 cccp 12",
        13: "20240816 mitomycinc 13",
        14: "20240820 cytochalasind 14",
        15: "20240821 lantrunculinb 15",
        16: "20240823 mdivi1 16",
        17: "20240826 nocodazole 17",
        18: "20240830 colchicine 18",
        19: "20240903 antimycina 19",
        20: "20240904 tiron 20",
        21: "20240905 cisplatin 21",
        22: "20240910 rotenone 22",
        23: "20240911 nigericin 23",
        24: "20240912 azide 24",
        25: "20240913 paraquat 25",
        26: "20240917 metformin 26",
    }
...
def minus_one_to_one_normalization(
...
def idxs_to_keep(
    x: Union[List[np.ndarray], np.ndarray], idxs: List[int] = None
...
def plot_confusion_matrix(
    ...
    plt.savefig(
        f"{checkpoint_path}/confusion_matrix_top_{top_n}_test_func_1_1", dpi=400
    )


def get_patches_around_nucleus(
    image, patch_size=128, contour_area=100, only_center=True
):
...
def increase_contrast(gray_images):
    ...
    if not isinstance(gray_images, torch.Tensor):
        image = (gray_images - gray_images.min()) / (
            gray_images.max() - gray_images.min()
        )
        equalized_image = cv2.equalizeHist((image * 255.0).astype(np.uint8))
    ...
    equalized_image = np.stack(
        [cv2.equalizeHist((x * 255.0).astype(np.uint8)) for x in images]
    )
...
def get_fpaths(root_dir, seed=1123):
    drug_labels = {}
    with open(
        "/home/earkfeld/Projects/MitoSpace4D/extraction_utils/drugs_to_labels.txt", "r"
    ) as f:
    ...
    drug_folders = sorted(
        [file for file in os.listdir(root_dir) if osp.isdir(osp.join(root_dir, file))]
    )
    all_filenames = []
    ...
    for drug_folder in drug_folders:
        ...
        filenames = sorted(
            [file for file in os.listdir(osp.join(root_dir, drug_folder))]
        )
