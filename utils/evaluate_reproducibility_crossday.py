...
def filter_by_label(pick_labels, embeddings, labels, drug_labels_dict, label_drug_dict, dates=None):
...
def run_fold(test_date, embeddings, labels, dates, drug_labels_dict, label_drug_dict,
             top_ns, cmat_title, args, cfg, crossval_dir):
    ...
    print(f"Train: {train_mask.sum()} samples  (dates={sorted(np.unique(dates[train_mask]).tolist())})")
    ...
    if args.dist_metric == 'cosine':
        ...
        dist_matrix, dist_matrix_idxs = cosine_distance(
            eval_embeddings, train_embeddings, weighted=False,
            temperature=cfg["training"]["loss"]["temperature"])
        preds, correct_preds_idxs, _ = nearest_neighbor_evaluation(
            eval_labels, train_labels, top_ns, dist_matrix,
            dist_matrix_idxs=dist_matrix_idxs, verbose=False)
        ...
        plot_cm(eval_labels, preds[1], label_drug_dict,
                verbose=False, show_values=False,
                title=cmat_title,
                save_path=cm_path)
        plt.close('all')

        cm_path_vals = osp.join(crossval_dir, f"cm_top1_with_values-test_date={test_date}.png")
        plot_cm(eval_labels, preds[1], label_drug_dict,
                verbose=False, show_values=True,
                title=cmat_title,
...
if __name__ == "__main__":
    ...
    if already_have_embeddings:
        ...
        label_drug_dict = {label: drug for label, drug in label_drug_dict.items()
                           if label in unique_labels_in_dataset}

        if args.labels:
            embeddings, labels, drug_labels_dict, label_drug_dict, dates = filter_by_label(
                args.labels, embeddings, labels, drug_labels_dict, label_drug_dict, dates=dates)

        if balance_classes:
            embeddings, labels, dates = balance_label_counts(embeddings, labels, dates=dates)
        ...
        if args.test_date and args.test_date in unique_dates:
            ...
            print(f"Running {len(fold_dates)}-fold leave-one-day-out cross-validation across: {fold_dates}")
        ...
        for test_date in fold_dates:
            ...
            per_drug_rows, fold_summary_rows, fold_top1_data = run_fold(
                test_date=test_date,
                embeddings=embeddings, labels=labels, dates=dates,
                drug_labels_dict=drug_labels_dict, label_drug_dict=label_drug_dict,
                top_ns=top_ns, cmat_title=cmat_title,
                args=args, cfg=cfg, crossval_dir=crossval_dir,
            )
        ...
        if all_per_drug_rows:
            ...
            mean_per_drug = (
                ...
                .agg(n_correct_sum=('n_correct', 'sum'),
            ...
            mean_per_drug['accuracy_micro'] = (
                mean_per_drug['n_correct_sum'] / mean_per_drug['n_total_sum'].replace(0, np.nan) * 100.0
            )
        ...
        if all_fold_summary_rows:
            ...
            mean_across = (fold_df.groupby(['dist_metric', 'top_n'])['accuracy']
                                  .agg(['mean', 'std', 'count'])
                                  .reset_index())
        ...
        if all_top1_gt:
            ...
            plot_cm(gt_concat, preds_concat, label_drug_dict,
                    verbose=False, show_values=True, normalize=False,
                    title=cmat_title,
                    save_path=sum_cm_path)
            plt.close('all')

            mean_cm_path = osp.join(crossval_dir, "cm_top1-mean_across_folds_normalized.png")
            plot_cm(gt_concat, preds_concat, label_drug_dict,
                    verbose=False, show_values=False, normalize=True,
                    title=cmat_title,
                    save_path=mean_cm_path)
            plt.close('all')

            mean_cm_vals_path = osp.join(crossval_dir, "cm_top1-mean_across_folds_normalized_with_values.png")
            plot_cm(gt_concat, preds_concat, label_drug_dict,
                    verbose=False, show_values=True, normalize=True,
                    title=cmat_title,
            ...
            per_fold_norm = np.full((n_folds, n_classes, n_classes), np.nan, dtype=float)
            ...
            for show_values, suffix in [(False, ""), (True, "_with_values")]:
                with plt.rc_context(CM_FONT_RC):
                    ...
                    ax.set_yticks(tickmarks)
                    ax.set_yticklabels(label_names)
                    im = ax.imshow(macro_mean_cm, cmap=plt.get_cmap('Blues'),
                    ...
                    if show_values:
                        thresh = macro_mean_cm.max() / 2.0 if macro_mean_cm.max() > 0 else 0.0
                        for i in range(n_classes):
                            for j in range(n_classes):
                                ax.text(j, i, f"{macro_mean_cm[i, j]:.2f}",
                                        horizontalalignment="center",
                                        color="white" if macro_mean_cm[i, j] > thresh else "black")
                    ax.set_ylabel('True label')
                    ax.set_xlabel('Predicted label')
                    macro_path = osp.join(crossval_dir,
                                          f"cm_top1-macro_mean_across_folds{suffix}.png")
                    fig.savefig(macro_path, bbox_inches='tight')
                    plt.close('all')
                    macro_paths.append(macro_path)

            np.save(osp.join(crossval_dir, "cm_top1-per_fold_normalized.npy"), per_fold_norm)
            np.save(osp.join(crossval_dir, "cm_top1-macro_mean_across_folds.npy"), macro_mean_cm)
        ...
        embeddings_umap = embeddings_umap / np.linalg.norm(embeddings_umap, axis=1)[:, None]
        ...
        kl_divergence = entropy(original_distances_normalized.flatten(),
        ...
        train_embeddings, _, train_labels, _ = extract_embeddings_from_model(
            train_loader, model.model, normalize_embeddings=True,
            get_images=False, get_labels=True, messup_tmrm=False,
            visualise_model_layer=False, get_fpaths=True)
        eval_embeddings, _, eval_labels, _ = extract_embeddings_from_model(
            eval_loader, model.model, normalize_embeddings=True,
            get_images=False, get_labels=True, messup_tmrm=False,
            visualise_model_layer=False, get_fpaths=True)
        ...
        if args.dist_metric == 'cosine':
            ...
            preds, _, _ = nearest_neighbor_evaluation(
                eval_labels, train_labels, top_ns, dist_matrix, dist_matrix_idxs=dist_matrix_idxs)
            plot_cm(eval_labels, preds[1], label_drug_dict,
                    verbose=False, show_values=False, title=cmat_title,
