...
def degrade_llsm_to_confocal(image_3d):
    ...
    img = np.mean(img_morph[mid_z - 5: mid_z + 5, :, :],
                   axis=0)  # use entire z-stack due to thick optical sectioning in cell painting
...
if __name__ == '__main__':
    root_path = '/mnt/aquila/ssd_processing/Others/MitoSpace4D/2024v3_data/processed_data/'
    ...
    for sample_path in all_sample_path:
        ...
        for cell_id, morph_file in tqdm(enumerate(all_files)):
            ...
            tmrm_zstack = np.load(morph_file.replace("-0-1.npy", "-0-0.npy"))[frame_index] # ch, frame 0

            zstack = np.stack([tmrm_zstack, zstack], axis=0) # ch, z, x, y; tmrm is now channel 0, morph is channel 1
