...
if __name__ == '__main__':
    root_path = '/mnt/aquila/ssd_processing/Others/MitoSpace4D/2024v2_data/processed_data/'
    ...
    for sample_path in all_sample_path:
        all_files = sorted(glob.glob(sample_path + '/*npy'))

        print('Processing', sample_path)
        for cell_id, file in tqdm(enumerate(all_files)):
            # zstack = np.load(file)[1, 0]
            zstack = np.load(file)[:, 0] # ch, frame 0
            ...
            # outfile = save_path + str(cell_id).zfill(6) + '.npy'
