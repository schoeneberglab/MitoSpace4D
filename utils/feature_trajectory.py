...
# # DATA_PATH = "/home/earkfeld/Projects/MitoSpace4D/experiments/dhruv_4D_ResnetBiLSTM_phate/kinetics-tscrambled_4DMS-2024v2_phate_mean-pooled.parquet"
# #
# # # OUTPUT_DIR = "/home/dhruvagarwal/projects/MitoSpace4D/temporal_experiments/3d_trajectories_kinetics_one_region"
# # OUTPUT_DIR = "/home/earkfeld/Projects/MitoSpace4D/experiments/dhruv_4D_ResnetBiLSTM_phate/2b_kinetics-tscrambled_4DMS-2024v2_phate_mean-pooled"
# #
...
#     """Create 3D trajectory plot for a single drug with modern styling"""
...
#     """Create combined 3D trajectory plot with modern UI controls"""
#     os.makedirs(output_dir, exist_ok=True)
#
#     avg_range = np.mean([
#         meta[embx].max() - meta[embx].min(),
#         meta[emby].max() - meta[emby].min(),
#         meta[embz].max() - meta[embz].min()
#     ])
#     cone_size_ref = avg_range * 0.012  # Much smaller arrows (was 0.05)
#
#     fig = go.Figure()
#     drug_trace_indices = {}
#     current_trace_idx = 0
...
#         shade_hex = [rgb_to_hex(shade) for shade in shades]
#
#         # Draw the full trajectory as a single continuous line
#         fig.add_trace(
#             go.Scatter3d(
#                 x=xs,
#                 y=ys,
#                 z=zs,
#                 mode="lines",
#                 line=dict(color=rgb_to_hex(base_color), width=2.5),  # Thinner lines
#                 opacity=0.8,
#                 showlegend=True,
#                 name=drug_name,
...
#         current_trace_idx += 1
#
#         # Only add ONE arrow at the end to show direction (much less clutter)
#         if num_t >= 2:
#             # Use the last segment for the arrow
#             i = num_t - 2
#             dx, dy, dz = xs[i + 1] - xs[i], ys[i + 1] - ys[i], zs[i + 1] - zs[i]
#             vec_len = np.sqrt(dx**2 + dy**2 + dz**2)
#             if vec_len > 0:
...
#                 fig.add_trace(
#                     go.Cone(
#                         x=[xs[-1]],  # At the end point
#                         y=[ys[-1]],
#                         z=[zs[-1]],
#                         u=[u],
#                         v=[v],
#                         w=[w],
#                         colorscale=[[0, rgb_to_hex(base_color)], [1, rgb_to_hex(base_color)]],
#                         sizemode="absolute",
#                         sizeref=cone_size_ref,
#                         showscale=False,
#                         showlegend=False,
...
#                 current_trace_idx += 1
#
#         # Start and end markers only (smaller)
#         fig.add_trace(
#             go.Scatter3d(
#                 x=[xs[0], xs[-1]],
#                 y=[ys[0], ys[-1]],
...
#                 mode="markers",
#                 marker=dict(
#                     size=[4, 5],  # Start smaller, end slightly larger
#                     color=[shade_hex[0], shade_hex[-1]],
#                     opacity=0.9,
...
#                 legendgroup=drug_name,
#                 name=drug_name,
#                 hovertemplate=f"<b>{drug_name}</b><br>Time: %{{text:.2f}}<br>X: %{{x:.3f}}<br>Y: %{{y:.3f}}<br>Z: %{{z:.3f}}<extra></extra>",
#                 text=[time_vals[0], time_vals[-1]],
#             )
#         )
#         drug_trace_indices[drug_name].append(current_trace_idx)
#         current_trace_idx += 1
...
#                             updatedData[idx].visible = isVisible ? true : 'legendonly';
#                         }}
#                     }});
#                 }}
#             }});
...
# # ===========================================================
#
# def generate_phate_trajectories(df, output_dir):
#
#     print("\n🎨 Generating 3D trajectory plots...")
#
#     embeddings = np.stack(df["phate_embeddings"].values)
#     embx = embeddings[:, 0]
#     emby = embeddings[:, 1]
#     embz = embeddings[:, 2]
#     # meta = df[["label_name", "time"]].copy()
#     meta = df[["label_name", "time", "cmap_label"]].copy()
#     meta.rename(columns={"label_name": "drug"}, inplace=True)
#
#     # Set up PHATE Columns
#     meta["PHATE1"] = embx
#     meta["PHATE2"] = emby
#     meta["PHATE3"] = embz
#
#
#     drug_colors = {}
#     for drug_name in df["label_name"].unique():
#         df_drug = df[df["label_name"] == drug_name]
#         color = df_drug["cmap_label"].values[0]
#         drug_colors[drug_name] = tuple(color)
#
#     for embx, emby, embz, prefix in [
#         ("PHATE1", "PHATE2", "PHATE3", "phate"),
#         # ("UMAP1", "UMAP2", "UMAP3", "umap")
#     ]:
#         print(f"\n  [{prefix.upper()}]")
#
...
def plot_3d_trajectories_combined(meta, embx, emby, embz, drug_colors, prefix, output_dir):
    ...
    fig.update_layout(
        ...
        plot_bgcolor=THEME["bg_color"],
        autosize=True,
        # Increased left margin to 260 to accommodate the 220px control panel
        margin=dict(l=260, r=200, t=80, b=20),
        legend=dict(
            yanchor="top",
            y=0.95,
            xanchor="left",
