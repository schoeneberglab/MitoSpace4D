...
# ----------------- Experiment -----------------
def temporal_sensitivity_experiment(model, video, plot=False):
    """
    video: (T,C,D,H,W)
    returns per-frame cosine similarity between original and shuffled embeddings
    """
    # shuffle video
    shuffled, perm = perturb_shuffle(video)
    shuffled_back = shuffled[np.argsort(perm)]  # restore original order

    # convert to batch
    orig_input = video.unsqueeze(0).float()
    shuffled_input = shuffled.float().unsqueeze(0)

    # get embeddings
    emb_orig = get_embeddings(model, orig_input)          # (T,D)
    emb_shuffled = get_embeddings(model, shuffled_input)  # (T,D)
    emb_shuffled_back = emb_shuffled    # align timesteps

    # per-timestep cosine
    cos_per_frame = cosine_per_timestep(emb_orig, emb_shuffled_back)
...
if __name__ == '__main__':
    ...
    for drug_folder in drug_folders:
        ...
        for idx in selected_indices:
            ...
            cosine_per_frame = temporal_sensitivity_experiment(model.model, data)
