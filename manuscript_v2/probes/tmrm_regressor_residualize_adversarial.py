...
def residualize_embeddings(X_train, X_val, z_train, z_val,
                           basis="spline", n_knots=6, degree=3):
    """Regress z out of each column of X via OLS on a (possibly nonlinear) basis
    expansion of z. Fit on train, apply the same projection to val.

    basis:
      - "linear":     project X onto [1, z]               (removes linear MT signal)
      - "polynomial": project onto [1, z, z^2, ..., z^d]  (d = `degree`)
      - "spline":     project onto a B-spline basis with  `n_knots` quantile knots
                      and the given `degree`              (GAM-style nonlinear removal)

    The basis is fit on train z only (e.g. spline knots placed at train quantiles)
    and then applied to val z — no val->train leakage.

    X: (N, D). z: (N,) or (N, K) nuisance. Returns (X_train_res, X_val_res)."""
    if z_train.ndim == 1:
        z_train = z_train.reshape(-1, 1)
        z_val = z_val.reshape(-1, 1)

    if basis == "linear":
        Phi_train, Phi_val = z_train, z_val
    elif basis == "polynomial":
        pf = PolynomialFeatures(degree=degree, include_bias=False)
        Phi_train = pf.fit_transform(z_train)
        Phi_val = pf.transform(z_val)
    elif basis == "spline":
        st = SplineTransformer(n_knots=n_knots, degree=degree, knots="quantile",
                               include_bias=False)
        Phi_train = st.fit_transform(z_train)
        Phi_val = st.transform(z_val)
    else:
        raise ValueError(f"Unknown basis '{basis}'. Must be 'linear', 'polynomial', or 'spline'.")

    # Prepend intercept column so OLS also absorbs the mean.
    Z_train = np.hstack([np.ones((Phi_train.shape[0], 1)), Phi_train])
    Z_val = np.hstack([np.ones((Phi_val.shape[0], 1)), Phi_val])
    # coef: (n_basis+1, D) — one column of coefficients per embedding dim
    coef, *_ = np.linalg.lstsq(Z_train, X_train, rcond=None)
    X_train_res = X_train - Z_train @ coef
    X_val_res = X_val - Z_val @ coef
...
def train_model(embeddings, targets, save_dir, regression_target="target", desc=None, conditions=None, labels=None,
    ...
    # Residualize embeddings against the nuisance variable (e.g. MitoTracker intensity).
    # Split nuisance with the same (test_size, random_state) so rows line up with X/y.
    # OLS fit happens on train only inside residualize_embeddings -> no val->train leakage.
    if nuisance is not None:
        nuisance_train, nuisance_val = train_test_split(
            nuisance, test_size=test_split, random_state=42
        )
        X_train, X_val = residualize_embeddings(
            X_train, X_val, nuisance_train, nuisance_val, basis=residualize_basis
        )
        print(f"Residualized embeddings against nuisance (shape: {nuisance.shape}, basis: {residualize_basis}).")
...
def predict_with_model(model, X_val, target_scaler):
    """Return predictions in the original target units."""
    model.eval()
    device = next(model.parameters()).device
    with torch.no_grad():
        preds_scaled = model(torch.FloatTensor(X_val).to(device)).cpu().numpy()
    return target_scaler.inverse_transform(preds_scaled).flatten()


def plot_residualization_check(y_target, preds_target, target_name,
                               y_nuisance, preds_nuisance, nuisance_name,
                               save_path, lims=(0, 1)):
    """Side-by-side scatter for target and nuisance probes trained on the same
    residualized embeddings. If residualization succeeded, the nuisance R² should
    collapse toward 0 while the target R² remains informative."""
    r2_t = r2_score(y_target, preds_target)
    r2_n = r2_score(y_nuisance, preds_nuisance)

    fig, axes = plt.subplots(1, 2, figsize=(12, 5), dpi=300)
    for ax, y, p, name, r2 in [
        (axes[0], y_target, preds_target, target_name, r2_t),
        (axes[1], y_nuisance, preds_nuisance, nuisance_name, r2_n),
    ]:
        sns.regplot(x=y, y=p, ax=ax,
                    scatter_kws={'alpha': 0.3},
                    line_kws={'color': 'red', 'alpha': 0.5},
                    ci=None)
        ...
        ax.set_xlim(lims)
        ax.set_ylim(lims)
        ax.plot(lims, lims, color='gray', linestyle=':', alpha=0.6)

    fig.suptitle('Residualization verification (probes on residualized embeddings)')
    fig.tight_layout()
    os.makedirs(osp.dirname(save_path), exist_ok=True)
    fig.savefig(save_path)
    plt.close(fig)
    print(f"Saved residualization-check plot to: {save_path}")
    print(f"  {target_name} R² = {r2_t:.4f}  |  {nuisance_name} R² = {r2_n:.4f}  "
          f"(low nuisance R² ⇒ residualization worked)")
...
def main(local_df,
    ...
    nuisance_vals = np.stack(local_df[nuisance_target].values) if nuisance_target is not None else None

    # 2. Get specified frame only
    if embeddings.ndim == 3:
        embeddings = embeddings[:, frame_index, :]
        target_vals = target_vals[:, frame_index]
        if nuisance_vals is not None and nuisance_vals.ndim == 2:
            nuisance_vals = nuisance_vals[:, frame_index]
        ...
        if nuisance_vals is not None:
            nuisance_vals = np.array([n[frame_index] for n in nuisance_vals])
...
if __name__ == "__main__":
    ...
    regression_target = "tmrm_intensities"
    nuisance_target = "morph_intensities"  # regress MitoTracker intensity out of each embedding dim; set to None to disable
    generate_plot = True
    ...
    desc = "frame=-1_residualized_morph_spline"
    ...
    training_cfg = {
        "regressor_type": "nonlinear",         # "linear" or "nonlinear"
        "feature_norm": "standardize",      # "standardize", "l2", or "none"
        "residualize_basis": "spline",      # "linear", "polynomial", or "spline"
        "epochs": 100,
        "batch_size": 2048,
        "test_split": 0.2,
