...
def evaluate_and_plot(model, X_val, y_val, val_conditions, val_labels, target_scaler, title=None, save_path=None,
    ...
    if generate_plot:
        ...
        if val_conditions is not None and len(raw_error_data) > 0:
            ...
            label_colors = get_label_colormap()
            unique_mapping = plot_df[['x_tick', 'Label']].drop_duplicates()
            palette = {row['x_tick']: label_colors.get(int(row['Label']), [0.5, 0.5, 0.5]) for _, row in unique_mapping.iterrows()}

            # Sort the dataframe and labels in increasing order according to "Absolute Error (%)"
            sort_map = {}
