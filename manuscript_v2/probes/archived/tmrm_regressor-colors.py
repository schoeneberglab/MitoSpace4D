...
def evaluate_and_plot(model, X_val, y_val, val_conditions, val_labels, target_scaler, title=None, save_path=None,
    ...
    if generate_plot:
        ...
        if val_labels is not None:
            color_map = get_label_colormap()
            # Map labels to colors. Fallback to a default grey if label not in colormap.
            point_colors = [color_map.get(int(lbl), [0.5, 0.5, 0.5]) for lbl in val_labels]

            # Scatter plot with per-label colors
            plt.scatter(y_val, preds_orig, c=point_colors, alpha=0.3)
            # Add regression line without overriding scatter colors
            sns.regplot(x=y_val, y=preds_orig, scatter=False, line_kws={'color': 'red'})
        else:
            sns.regplot(x=y_val, y=preds_orig, scatter_kws={'alpha': 0.3}, line_kws={'color': 'red'})
