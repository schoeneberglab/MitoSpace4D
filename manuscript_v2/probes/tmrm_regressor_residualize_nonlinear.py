...
def residualize_embeddings(X_train, X_val, z_train, z_val):
    """Regress z out of each column of X via OLS. Fit on train, apply the same linear
    projection to val (so val embeddings are transformed without their own targets
    leaking into the fit). Returns (X_train_res, X_val_res).

    X: (N, D) embeddings. z: (N,) or (N, K) nuisance variable(s)."""
    if z_train.ndim == 1:
    ...
    # coef: (K+1, D) — one column of coefficients per embedding dim
...
def train_model(embeddings, targets, save_dir, regression_target="target", desc=None, conditions=None, labels=None,
    ...
    if nuisance is not None:
        ...
        X_train, X_val = residualize_embeddings(X_train, X_val, nuisance_train, nuisance_val)
        print(f"Residualized embeddings against nuisance variable (shape: {nuisance.shape}).")
...
if __name__ == "__main__":
    ...
    desc = "frame=-1_residualized_morph"
