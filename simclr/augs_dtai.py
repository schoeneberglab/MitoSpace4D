...
class RandomBrightness(nn.Module):
    def __init__(self, p=0.0, lower=-0.2, upper=0.2, per_channel=False) -> None:
        super().__init__()
        self.p = p
...
class DataAugmentation(nn.Module):
    def __init__(self, cfg_aug=None, zero_mean_norm=True, n_views=2) -> None:
        ...
        self.brightness_transform = RandomBrightness(p=cfg_aug['RandomBrightness']['p'],
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.n_views):
            views[i] = self.brightness_transform(views[i])  # apply brightness transform separately
...
if __name__ == "__main__":
    rand_brightness = RandomBrightness(p=1.0, lower=-0.2, upper=0.2, per_channel=True)

    import numpy as np
    import einops
    img = np.load("/mnt/aquila/SSD_processing/Others/MitoSpace4D/2025_summer_new/20250722-2/000001-0.npy")
    x = torch.from_numpy(img).unsqueeze(0).to(torch.float32)  # (1, c, t, z, h, w)
