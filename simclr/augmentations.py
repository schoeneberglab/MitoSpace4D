...
class RandomTimeMask(nn.Module):
    def __init__(self, p=0.5, n_views=2) -> None:
        super().__init__()
        self.p = (
            p  # p actually doesn't matter here; only when it's zero, we don't apply
        )
        self.n_views = n_views

        self.time_delay = torch.Tensor([16, 14, 12, 10, 8, 6, 4, 2, 0])
        self.probs_time_delay = torch.Tensor(
            [0.03, 0.038, 0.047, 0.059, 0.074, 0.092, 0.115, 0.144, 0.401]
        )

        self.clip_len = torch.Tensor([20, 19, 18, 17, 16, 15, 14, 13, 12, 11])
        self.clip_len_probs = torch.Tensor(
            [0.501, 0.116, 0.092, 0.074, 0.059, 0.047, 0.038, 0.030, 0.024, 0.019]
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        if x.size(1) > clip_len:
            start = torch.randint(
                0, (x.size(1) - clip_len), (1,), device=x.device
            ).item()
        else:
            start = 0
        end = start + clip_len
        ...
        if self.n_views == 1:
            ...
            td_probs = torch.as_tensor(
                self.probs_time_delay, dtype=torch.float, device=x.device
            )
            td_vals = torch.as_tensor(
                self.time_delay, dtype=torch.long, device=x.device
            )
            ...
            raise NotImplementedError(
                "Only n_views=1 or 2 is supported for RandomTimeMask."
            )
...
class RandomBrightness(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mult_factors = (
            ...
            * (self.mult_upper - self.mult_lower)
            + self.mult_lower
        )
...
class DataAugmentation(nn.Module):
    def __init__(self, cfg_aug=None, zero_mean_norm=True, n_views=1) -> None:
        ...
        self.temporal_transform_1 = RandomTimeMask(
            p=cfg_aug["RandomTimeMask"]["p"], n_views=self.n_views
        )
        self.temporal_transform_2 = RandomTimeFlip(
            p=cfg_aug["RandomTimeFlip"]["p"]
        )  # )  # Always set to 0; kept for completeness. We do not reverse the time dimension, since a time-reversed sequence should not be treated as a positive pair.

        self.transforms_2d = nn.Sequential(
            RandomResizedCrop(
                p=cfg_aug["ResizedCrop"]["p"],
                size=(cfg_aug["ResizedCrop"]["size"], cfg_aug["ResizedCrop"]["size"]),
                scale=(
                    cfg_aug["ResizedCrop"]["scale"][0],
                    cfg_aug["ResizedCrop"]["scale"][1],
                ),
            ),
            RandomGaussianBlur(
                p=cfg_aug["GaussianBlur"]["p"],
                kernel_size=(
                    cfg_aug["GaussianBlur"]["kernel_size"],
                    cfg_aug["GaussianBlur"]["kernel_size"],
                ),
                sigma=(
                    cfg_aug["GaussianBlur"]["sigma"],
                    cfg_aug["GaussianBlur"]["sigma"],
                ),
            ),
            RandomErasing(
                p=cfg_aug["RandomErasing"]["p"],
                scale=(
                    cfg_aug["RandomErasing"]["scale"][0],
                    cfg_aug["RandomErasing"]["scale"][1],
                ),
                ratio=(
                    cfg_aug["RandomErasing"]["ratio"][0],
                    cfg_aug["RandomErasing"]["ratio"][1],
                ),
            ),
            RandomGaussianNoise(
                p=cfg_aug["GaussianNoise"]["p"],
        ...
        self.transforms_3d = nn.Sequential(
            ...
            RandomRotation3D(
                p=cfg_aug["RandomRotation3D"]["p"],
                degrees=cfg_aug["RandomRotation3D"]["degrees"],
            ),
            RandomAffine3D(
                p=cfg_aug["RandomAffine3D"]["p"],
                degrees=cfg_aug["RandomAffine3D"]["degrees"],
                translate=(
                    cfg_aug["RandomAffine3D"]["translate"][0],
        ...
        self.brightness_3d = RandomBrightness(
            p=cfg_aug["RandomBrightness"]["p"],
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.n_views):
            ...
            views[i] = self.transforms_3d(views[i].reshape(b, t * c, z, h, w)).reshape(
                b, t, c, z, h, w
            )
