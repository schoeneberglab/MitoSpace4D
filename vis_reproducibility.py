    ...
    def _pick_points():
        ...
        if cmap == "label":
            ...
            if len(np.unique(col_values)) < 1000 and (isinstance(col_values[0], str) or isinstance(col_values[0], int)):  # arbitrary threshold for categorical vs continuous
                ...
                if cmap == "fragment_diffusivity_mean":
                    vmax_val = np.percentile(col_values, 99)
                    vmin_val = np.percentile(col_values, 1)
                    col_values = np.clip(col_values, vmin_val, vmax_val)
        ...
        for idx in idxs:
            if picked_image_paths is not None:
                ...
                img = np.load(df['image_paths'].iloc[idx])  # (t, c, d, h, w)

                print(img.shape)
                if img.shape == (2, 256, 256): # Check if image is 2D
                    # take only channel 1
                    imgs.append(img[1, ...])  # (h, w)
                else:
                    imgs.append(img[-1, ...].max(axis=0))  # MIP of last frame
                # if is_4d:
        ...
        for i in range(len(imgs)):
            ...
            axarr[i].imshow(imgs[i], cmap=plt.cm.viridis)
            # axarr[1, i].imshow(imgs[i][mito_idx], vmin=0., vmax=1., cmap=plt.cm.viridis)

            # vals.append(np.mean(imgs[i][:, :, 0]))
            vals.append(np.mean(imgs[i]))
            axarr[i].set_xticks([])
            # for minor ticks
            axarr[i].set_yticks([])
        ...
        for i, idx in enumerate(idxs):
            l = label_drug_dict[df['labels'][idx]]
        ...
        # Show non-blocking and wait ONLY for the 'f' figure to close
        plt.show(block=False)
...
if __name__ == '__main__':
    ...
    datafile = "embeddings+metadata.parquet"
