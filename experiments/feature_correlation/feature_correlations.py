...
def plot_pc_grid(pacf_results, acf_mean, acf_std, title_prefix):
    ...
    for idx, pc_name in enumerate(pcs):
        ...
        conf = 1.96 / np.sqrt(N * T)  # Rough approximation using total points
...
try:
    data = np.load(
        "/runs/20260111_pten_embeddings_resnet3d-kinetics-300eps_ablated-tmrm_modified_labels/embeddings_raw.npy")
    N, T, D = data.shape
...
vector_acf_mean, vector_acf_std = compute_batch_vector_acf(data, max_lag=9)

# B. Run Multi-PC PACF
pc_results = compute_multi_pc_pacf(data, n_components=12, max_lag=9)

# ---------------------------------------------------------
# 3. Plot Both
# ---------------------------------------------------------
plot_pc_grid(pc_results, vector_acf_mean, vector_acf_std, "Embeddings Analysis")
plt.show()