...
if __name__ == "__main__":
    ...
    drugs = [label_drug_dict[label] for label in conditions]

    w2_matrix = np.zeros((len(conditions), len(conditions)))
    p_matrix = np.zeros((len(conditions), len(conditions)))
    ...
    # Populate the matrix
    for i, xlabel in enumerate(conditions):
        for j, ylabel in enumerate(conditions):
            x = embeddings[labels == xlabel]
            y = embeddings[labels == ylabel]
            Sxy, p = sw2(x, y,
                           pca_dim=pca_dim,
                           n_proj=n_proj,
                           n_perm=n_perm,
    ...
    # Save the matrices
    np.save(f"{embeddings_dir}/w2_distance_matrix.npy", w2_matrix)
    np.save(f"{embeddings_dir}/w2_pvalue_matrix.npy", p_matrix)

    # Print the matrices
    print(f"Wasserstein Distance Matrix:\n{w2_matrix}")
    print(f"P-value Matrix:\n{p_matrix}")

    # Plot the matrices with the seaborn heatmap function with drugs as
    sns.heatmap(w2_matrix, annot=True, fmt=".2f", cmap="Blues", xticklabels=drugs, yticklabels=drugs)
