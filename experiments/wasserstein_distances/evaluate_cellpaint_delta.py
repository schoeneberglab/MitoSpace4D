...
from utils.wasserstein_distance4 import *
...
if __name__ == "__main__":
    ...
    embeddings = np.load(
        f'{embeddings_dir}/{dataset}/embeddings.npy')
    labels = np.load(
        f'{embeddings_dir}/{dataset}/labels.npy')
    ...
    X_normalized = normalize(X_scaled, norm='l2', axis=1)
    embeddings = X_normalized

    # Rescale to [0, 1] for stability and using global min/max to preserve the geometric relationship between all points
    # emb_min = embeddings.min()
    # emb_max = embeddings.max()
    # embeddings = (embeddings - emb_min) / (emb_max - emb_min)

    # Print the min/max magnitude for the embedding vectors
    print(f"Min embedding magnitude: {np.min(embeddings):.6f}")
    print(f"Max embedding magnitude: {np.max(embeddings):.6f}")
    # pca_dim = get_pca_dim(embeddings)
    ...
    print(len(A_embeddings), len(A_prime_embeddings))
    ...
    # EXPERIMENT V4
    # s_obs, p_val, z = sw2(A_embeddings, A_prime_embeddings, pca_dim=pca_dim, n_perm=n_perm, n_proj=n_proj)
    # print(f"Observed SW2: {s_obs:.4f}")
    # print(f"P-value: {p_val:.4f}")
    # print(f"Sensitivity (Z-score): {z:.2f}")

    # EXPERIMENT V5
    s_obs, p_val, z = w2(A_embeddings, A_prime_embeddings, n_perm=n_perm)
    print(f"Observed SW2: {s_obs:.4f}")
    print(f"P-value: {p_val:.4f}")
    print(f"Sensitivity (Z-score): {z:.2f}")
