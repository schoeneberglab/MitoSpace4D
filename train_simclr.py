...
def main():
    ...
    if cfg["training"]["continue_from_ckpt_wo_opt"] != "None":
        ...
        train_runner = SimCLRRunner.load_from_checkpoint(
            cfg["training"]["continue_from_ckpt_wo_opt"], cfg=cfg, model=model
        )
        print(
            f"Loading checkpoints without optimizer from {cfg['training']['continue_from_ckpt_wo_opt']}"
        )
