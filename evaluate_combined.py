...
parser.add_argument(
    "--save_embeddings", default=True, action="store_true", help="Save embeddings"
)
parser.add_argument(
    "--save_tmrm_intensities",
    default=False,
    action="store_true",
    help="Save TMRM intensities",
)
...
def generate_embeddings(args, cfg, embeddings_dir, drug_labels_dict, label_drug_dict):
    ...
    emb_resnet_path = osp.join(
        embeddings_dir, "embeddings_resnet.npy"
    )  # (N, 512) float32
    ...
    if args.random_init:
        ...
        model = SimCLRRunner.load_from_checkpoint(
            checkpoint_path, model=model, cfg=cfg, strict=False
        ).model
        ...
        for i, batch in enumerate(loader):
            ...
            if args.vary_intensity > 0:
                scale = 1.0 + np.random.uniform(
                    -args.vary_intensity, args.vary_intensity
                )
            ...
            with torch.no_grad():
                ...
                features, resnet_features, _ = model(
                    torch.from_numpy(im).to(device), get_resnet_feats=True
                )
            ...
            resnet_embeddings_list.extend(
                resnet_features.detach().cpu().numpy().astype(np.float32)
            )
    ...
    np.save(
        osp.join(embeddings_dir, "label_names.npy"),
...
def plot_cm(
    ...
    if make_plot:
        ...
        im = ax.imshow(cm_plot, cmap=cmap, interpolation="nearest", vmin=0.0, vmax=1.0)
        ax.set_title(title)
...
if __name__ == "__main__":
    ...
    if not already_have_embeddings:
        ...
        generate_embeddings(
            args, cfg, embeddings_dir, drug_labels_dict, label_drug_dict
        )
    ...
    if args.labels:
        embeddings, labels, drug_labels_dict, label_drug_dict = filter_by_label(
            args.labels, embeddings, labels, drug_labels_dict, label_drug_dict
        )
    ...
    train_embeddings, train_labels, eval_embeddings, eval_labels = split_dataset(
        embeddings, labels, split_perc=split_perc, balanced=balanced_split
    )
    ...
    if args.dist_metric == "cosine":
        ...
        preds, correct_preds_idxs, incorrect_preds_idxs = nearest_neighbor_evaluation(
            eval_labels,
            train_labels,
            top_ns,
            dist_matrix,
        ...
        for top_n in top_ns:
            ...
            df = pd.DataFrame(
                columns=["drug", "label", "n_correct", "n_total", "accuracy"]
            )
            # print per class accuracy
            for lbl in np.unique(train_labels):
                total = np.sum(eval_labels == lbl)
    ...
    if args.dist_metric == "l2":
        ...
        for k in num_neighbors:
            ...
            for top_n in top_ns:
                ...
                print(
                    f"------------------Top-{top_n} Evaluation for {k} Neighbors------------------------"
                )
