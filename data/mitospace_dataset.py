...
class MitoSpaceDataset(Dataset):
    def __init__(
        ...
        with open(
            "/home/earkfeld/Projects/MitoSpace4D/extraction_utils/drugs_to_labels.txt",
            "r",
        ) as f:
        ...
        drug_folders = sorted(
            [
                ...
                if osp.isdir(osp.join(self.root_dir, file))
            ]
        )

        if pick_labels is not None:
            # filter drug_folders to only include those that have labels in pick_labels
            drug_folders = [
                folder
                for folder in drug_folders
                if drug_labels[folder]["label"] in pick_labels
            ]
            print(
                f"Filtered drugs: {[drug_labels[folder]['drug'] for folder in drug_folders]}"
            )
        ...
        for drug_folder in drug_folders:
            filenames = sorted(
                [
                    file
                    for file in os.listdir(osp.join(self.root_dir, drug_folder))
                    if osp.isfile(osp.join(self.root_dir, drug_folder, file))
                    and file.endswith("-0-1.npy")
                ]
            )  # Morphology channel only
            filenames = [
                osp.join(self.root_dir, drug_folder, file) for file in filenames
            ]
        ...
        self.all_filenames, self.all_labels = (
            list(self.all_filenames),
            list(self.all_labels),
        )

        self.len_all_data = round(len(self.all_labels) * 1.0)
        ...
        if flag == "all":
            print("Using all the data without shuffling or splitting.")
        ...
        #     self.labels = self.all_labels[self.train_split + self.val_split:]
            ...
            if flag == "train":
                ...
                self.filenames = self.all_filenames[
                    self.train_split : self.train_split + self.val_split
                ]
                self.labels = self.all_labels[
                    self.train_split : self.train_split + self.val_split
                ]
                ...
                raise ValueError(
                    f"Invalid flag: {flag}. Must be one of 'train', 'val', 'test', or 'all'."
                )
        ...
        if pick_labels is not None:
            ...
            idxs_to_keep = [
                i for i, lbl in enumerate(self.labels) if lbl in pick_labels
            ]
            self.filenames = [self.filenames[i] for i in idxs_to_keep]
            self.labels = [self.labels[i] for i in idxs_to_keep]

        print(
            "Loading {} labels, found {} samples ...".format(
                len(np.unique(self.labels)), len(self.filenames)
            )
        )
