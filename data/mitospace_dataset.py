...
class MitoSpaceDataset(Dataset):
    def __init__(self, 
        ...
        for drug_folder in drug_folders:
            filenames = sorted([file for file in os.listdir(osp.join(self.root_dir, drug_folder)) if osp.isfile(osp.join(self.root_dir, drug_folder, file)) and file.endswith("-0-1.npy")]) # Morphology channel only
