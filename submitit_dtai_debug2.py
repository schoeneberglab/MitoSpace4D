...
- NEW: Optional `strace` wrapping via --use-strace; per-rank, per-pid logs into JOB_DIR/strace/.
"""
import argparse
import atexit
import logging
...
def parse_args():
    ...
    # strace
    parser.add_argument("--use-strace", action="store_true",
                        help="Launch training under strace and save per-process logs to JOB_DIR/strace/")
    parser.add_argument("--strace-syscalls", type=str, default="",
                        help="Comma-separated syscall filters for -e trace=..., e.g., 'file,network'. Empty = trace all.")
    parser.add_argument("--strace-string-len", type=int, default=128,
                        help="Max string size captured by strace (-s).")
...
class Trainer(object):
    def __init__(self, args):
        ...
        self._child_pgid = None  # for signal forwarding

    def _install_signal_forwarding(self):
        def _handler(signum, _frame):
            try:
                if self._child_pgid is not None:
                    os.killpg(self._child_pgid, signum)
            finally:
                # Exit promptly so Slurm can requeue if configured
                sys.exit(0)
    ...
    def __call__(self):
        ...
        # We execute train_simclr.py as a child process so we can optionally wrap with strace.
        train_cmd = [sys.executable, "train_simclr.py", "--config", str(self.args.config)]
        if self.args.log_every_n_steps is not None:
            train_cmd += ["--log-every-n-steps", str(self.args.log_every_n_steps)]

        # --- Optional strace wrapper ---
        cmd = train_cmd
        if self.args.use_strace:
            strace_dir = Path(self.args.job_dir) / "strace"
            strace_dir.mkdir(parents=True, exist_ok=True)
            base = strace_dir / f"strace_j{job_env.job_id}_r{job_env.global_rank}"
            strace_cmd = [
                "strace",
                "-f",            # follow forks
                "-ff",           # split output per PID: <base>.<pid>
                "-tt",           # absolute timestamps
                "-T",            # show time spent in syscalls
                "-s", str(int(self.args.strace_string_len)),
                "-o", str(base),
            ]
            if self.args.strace_syscalls:
                strace_cmd[1:1] = ["-e", f"trace={self.args.strace_syscalls}"]
            cmd = strace_cmd + train_cmd
            logger.info(f"Launching under strace; logs at {base}.*")

        # --- Run child and forward signals to its process group ---
        self._install_signal_forwarding()
        try:
            # Start child in its own process group so we can forward signals to all descendants.
