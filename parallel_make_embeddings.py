...
if __name__ == "__main__":
    ...
    def run_validate_saved_model(cfg, model_path, device, visualize_umap, save_embeddings, save_umap_path, concatenate_embeddings, folder, selected_idxs):
        cfg_ = cfg  # passed by reference; be careful if threading
        cfg_.val_filepaths_2 = [cfg_.val_filepaths[i] for i in selected_idxs]
        print(f"Folder: {folder} | Batch size: {len(cfg_.val_filepaths_2)}")
        ...
        for folder in pick_folders:
            ...
            for start in range(0, len(idxs), batch_size):
                ...
                if not selected_idxs:
                    continue
                # Deepcopy cfg for each worker to keep configuration isolated, especially val_filepaths_2
                import copy
                cfg_copy = copy.deepcopy(cfg)
                # each task is a submit call to ProcessPoolExecutor
                future = executor.submit(
                    run_validate_saved_model,
                    cfg_copy, model_path, device, visualize_umap, save_embeddings,
                    save_umap_path, concatenate_embeddings, folder, selected_idxs
                )
                tasks.append(future)

        # Optionally, wait for all futures to complete and collect results/exceptions
        for future in concurrent.futures.as_completed(tasks):
            try:
                future.result()
            except Exception as exc:
                print(f"Exception occurred in parallel task: {exc}")